...
def build_model():
    ...
    model.add(layers.Conv2D(384, kernel_size = (3,3), activation='linear', padding='valid', name='conv3'))
    model.add(LeakyReLU(alpha=0.01))

    model.add(layers.Conv2D(384, kernel_size = (3,3), activation='linear', padding='valid', name='conv4'))
    model.add(LeakyReLU(alpha=0.01))

    model.add(layers.Conv2D(256, kernel_size = (3,3), activation='linear', padding='valid', name='conv5'))
    model.add(LeakyReLU(alpha=0.01))
